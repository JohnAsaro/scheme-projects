;Production system where robot considers low stable obstacles to be equal to free spaces
...
(define rules 
  '(
    (r1 (if (current x) (adjacent y) (obstacle y) (stable y) (height y low) (not safe y)) ;if y is stable low
        (add (safe y))) ;y is safe
    (r2 (if (current x) (adjacent y) (not obstacle y) (not safe y)) ;if y is not an obstacle
      (add (safe y))) ;x is safe
    (r3 (if (current x) (adjacent y) (not path y) (not visited y) (safe y)) ;if y is adjacent and safe
        (delete (current x))
        (add (path x) (delete_adjacents) (move_to y))) ;move to y
    (r4 (if (current x) (path y) (adjacent y) (not visited y) (safe y)) ;if y is adjacent and safe
        (delete (current x) (path x))
        (add (visited x) (delete_adjacents) (backtrack_to y))) ;backtrack to y       
