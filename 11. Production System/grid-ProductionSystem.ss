;Production system where robot favors free spaces over low stable obstacles
...
(define rules 
  '(
    ...
    (r2 (if (current x)(adjacent y) (not path y) (not visited y) (obstacle y) (stable y) (height y low)) ;if y is a low stable obstacle
        (delete (current x))
        (add (path x) (delete_adjacents) (move_to y)))
    (r3 (if (current x) (path y) (adjacent y) (not obstacle y))
        (delete (current x) (path x))
        (add (visited x) (delete_adjacents) (backtrack_to y)))     
    (r4 (if (current x) (path y) (adjacent y) (obstacle y) (stable y) (height y low))
        (delete (current x) (path x))
        (add (visited x) (delete_adjacents) (backtrack_to y)))       
