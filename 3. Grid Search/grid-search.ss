...
(define search2
  (lambda (grid count stop-count)
    ...
    (let ((x (robot-x))
      ...
      (set-node! grid (robot-x) (robot-y) 1000)
      (if (or
...
(define move-robot
  (lambda (grid x y count)
    (let ((dir (random 4)))
      (cond
        ((and (= dir n) (> x 0) (< (get-node grid (- x 1) y) 1000))
           (set! robot (list (- x 1) y)))
        ((and (= dir s) (< x (- num-col-row 1)) (< (get-node grid (+ x 1) y) 1000))
           (set! robot (list (+ x 1) y)))
        ((and (= dir w) (> y 0) (< (get-node grid x (- y 1)) 1000))
           (set! robot (list x (- y 1))))
        ((and (= dir e) (< y (- num-col-row 1)) (< (get-node grid x (+ y 1)) 1000))
           (set! robot (list x (+ y 1))))
        ((> count 100)
           (display "stuck")
           (move-robot-2 grid x y 0))
        (else
