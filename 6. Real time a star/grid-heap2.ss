;Not an actual heap but doesnt heap just sort of roll off the tongue better than
;sorted array? 
...
(define dist-start-goal2
  ...
  (newline)
  (display node)
      (+ (dist-pt2 node goal) (get-steps-count2 node))))
...
(define hierarchy2 ;just to make heapify2 more readable
  (lambda (node-1 node-2)
    ...
    (display node-1)
    (display node-2)
    (< (dist-start-goal2 node-1) (dist-start-goal2 node-2))))
